...
|---------|------------|-------------|
| v49 | 49 | Billboard cache: changed from per-day cache to 30-minute TTL so new/edited campaigns appear within 30 min (was: cached all day, so campaigns created after first app-open that day were invisible); Cold-launch billboard fix: added `pendingBillboardPrayer` ref — if notification/deep-link fires `showBillboardForPrayer` before billboard config loads (cold start), the prayer key is queued and fired automatically when config arrives; BillboardAdminScreen: image thumbnail shows bg-colour placeholder when no image, with onError handler for CDN delay |
| v48 | 48 | BottomBar: Bank Transfer first / Donate Online second in donate modal (heart icon on main screen); QiblaScreen: body converted to ScrollView — compass at top, instructions scrollable below; header elevation removed so it no longer clashes with system menu bar; Magnetometer subscription starts immediately on screen open (independent of location permission) so compass ring always spins; permission denied now shows "Open App Settings" button (Linking.openSettings); WorldTimesScreen: unpin star (☆) font 18→28 to match pinned gold star; pre-pin hint corrected from ⭐ to ☆; Mecca/Madina city label font 11→13; dual temp row no longer full-width so temp value sits immediately after city name (≤3 spaces); data/worldTimes: exchange rate fallback chain — FloatRates → fawazahmed0 CDN → open.er-api.com (3 live sources); each source has 8s timeout; CountdownStrip: new `countdownMode` prop — shows small "ADHAN" or "IQAMAH" label beneath prayer name in countdown phase; App.tsx passes `settings.countdownMode` to CountdownStrip; AlertsScreen: `isAdmin` + `onTestBillboard` props added to TestAlarmSection — admin sees 📢 button per prayer that fires billboard preview; App.tsx: AsyncStorage check for `@eeis_admin_unlocked` on mount, passes `isAdminUnlocked` and `showBillboardForPrayer` to AlertsScreen; data/billboards: `ScrollingMessage` type added; `scrollingMessages?` field on `BillboardConfig`; `getActiveScrollingMessages()` helper; BillboardAdminScreen: "📣 Messages" tab added — view/delete saved messages, add new message with text/prayers/days/date-range form; App.tsx: activeHeadlines includes active scrolling messages for current next prayer from billboard config |
| v47 | 47 | HamburgerMenu: Bank Transfer first / Donate Online second in donate sub-menu; version footer "v47 · 28 May 2026" at bottom of drawer (from `constants/buildInfo.ts`); Countdown fix: `minutesUntilBegins` added to `NextPrayer` type — adhan mode now correctly counts to begins time without wrapping to 24h when begins has passed; App-open billboard fallback: when billboard config loads and user opened app within 30 min of a prayer time, billboard fires automatically; Qibla instructions rewritten: clearer 4-step guide with flat-surface emphasis; WorldTimesScreen: timezone group headers removed → thin separator lines between groups (pinned cities keep "⭐ Pinned Cities" label); country font +2pt (14→16), city sub-name +2pt (10→12), red offset label +2pt (14→16); Saudi Arabia dual temperature moved inline below city name (not right column); prayer time row baseline-aligned (was centre); exchange rates: 8-second timeout + stale cache fallback on network failure; calculator: ÷ division operator added; 2-row operator layout (+/− row 1, ×/÷ row 2, % and =/C on row 2); "Start" label removed from log first row and WhatsApp message; WhatsApp share includes rate + date; currency dropdown on local field to override city's default currency; currency picker overlay (alphabetical list with rate); BillboardAdminScreen help guide: added "What users see" and "Frequency limits" cards |
